...
1. Go to to the `Variables and Secrets`section
   - Add `UMAMI_WEBSITE_ID`Secret based from the `Website ID` of `www.vanityURLs.link` in [Umami](umami.is) account

1. Load [VanityURLs.link](VanityURLs.link) in your primary browser and [Umami](umami.is) in another browser, let's explain why we need two browsers during AI review

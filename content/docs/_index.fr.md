...
Commencez par le demarrage rapide pour creer un nouveau domaine court. Utilisez les pages de reference pour personnaliser l'instance par defaut ou verifier comment v8s.link est configure.

Pour durcir une production, lisez l'approche securite du runtime avec le guide Cloudflare et [Protection reseau](/fr/docs/customize/network-protection/). Le design repose sur un petit Worker, un registre genere, la validation au build, Cloudflare Access pour les vues privees, des controles reseau Cloudflare pour les abus courants, et des analytics serveur seulement pour le trafic qui atteint le Worker.

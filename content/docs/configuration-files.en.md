...
| `custom/v8s-site-config.json` | Instance-owned site settings, including languages, branding, operator contacts, and legal-page mode |
| `custom/v8s-links.txt` | Instance-owned short links |
...
`custom/v8s-site-config.json` is the main setup file written by `npm run setup`. The important top-level sections are:
...
Do not edit generated files in `build/`. Edit `custom/`, then rebuild.

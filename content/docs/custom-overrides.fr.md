...
Pour le raisonnement de mise a jour, lisez [Garder vanityURLs facile a mettre a jour avec custom](/fr/blog/keeping-vanityurls-upgradable-with-custom-overrides/). Pour l'ordre de build et les artefacts generes, lisez [Fichiers de configuration](/fr/docs/configuration-files/).
...
| `custom/v8s-site-config.json` | Reglages de site ecrits par setup | [Fichiers de configuration](/fr/docs/configuration-files/) |
| `custom/v8s-local-config.json` | Chemins de helper propres au poste | [Helper local](/fr/docs/local-helper/) |
...
Lorsque vous utilisez `custom/public/`, gardez `i18n.supported_languages` aligne avec les pages localisees que vous supportez vraiment. Voir [Internationalisation](/fr/docs/i18n/) pour les regles de repertoires de langue.
...
| Shell du tableau admin | `custom/public/_stats/index.html` | [Tableau admin](/fr/docs/admin-dashboard/) et [Controle d'acces](/fr/docs/access-control/) |
| Headers | `custom/public/_headers` | [Approche securite du runtime](/fr/docs/runtime-security/) |
...
Les versions localisees utilisent le [code langue](/fr/docs/i18n/#langues-supportees) comme premier segment de repertoire, par exemple `custom/public/fr/404.html`. Vous devez seulement ajouter les pages localisees que vous supportez vraiment. Si une page localisee manque, le Worker peut revenir a la page par defaut pour l'etat demande.

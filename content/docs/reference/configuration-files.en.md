...
| --------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `build/v8s.json`                  | Runtime link registry JSON, `schema_version: "3.1"`         | `generated_at`, `generated_timezone`, `default_state`, `routing`, `tree`; tree links have `slug`, `match`, `target`, `state`, metadata, and optional `schedule` | `scripts/build-redirect-targets.mjs`    |
| `build/v8s-blocklist.json`        | Runtime policy JSON, `schema_version: "1.0"`                | Selected source policy from `custom/v8s-policies.json` or `defaults/v8s-policies.json`, plus optional generated feed data                                       | `scripts/build.mjs`                     |
...
| `build/v8s-custom-assets.json`    | Runtime custom asset manifest JSON, `schema_version: "1.0"` | Final public paths copied from `custom/public/` so the Worker can apply the custom HTML security profile                                                        | `scripts/build.mjs`                     |
...
6. Build `v8s.json`, `v8s-blocklist.json`, `v8s-site-config.json`, and `v8s-custom-assets.json`
7. Generate `src/` from `scripts/workers/` for Wrangler

## Runtime artifacts

The Worker does not read the source link registry, `v8s-links.txt`, on every request. The build creates runtime artifacts from source files, validates them, and deploys them with the Worker assets.

Build inputs include:

- `defaults/v8s-links.txt`, replaced by the source link registry `custom/v8s-links.txt` when present, including inline `@schedule` blocks
...
| Artifact                       | Purpose                                                            |
| ------------------------------ | ------------------------------------------------------------------ |
| `build/v8s.json`               | Runtime link registry consumed by the Worker                       |
...
| `build/v8s-custom-assets.json` | Custom public asset manifest used by the Worker security profiles  |
| `src/worker.mjs`               | Generated Worker entry copied from `scripts/workers/` for Wrangler |

`scripts/workers/` is the Worker source of truth. `src/` is generated output. Direct public requests for raw runtime files such as `/v8s.json`, `/v8s-blocklist.json`, `/v8s-site-config.json`, and `/v8s-custom-assets.json` should return 404.

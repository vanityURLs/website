...
| Protected operational paths (`/<lang>/_stats`, `/<lang>/_tests`) | Authenticated maintainer                 | Worker, behind Cloudflare Access                    | Fails closed: `503` when Access is unconfigured, `403` without a valid JWT.                                                                                           |
...
| Private runtime assets hidden (`v8s.json`, blocklist, site config, custom asset manifest)                                             |         -          |          check (`404`)          |              check (`_headers` no-index/no-store fallback)              |
...
| Lookup anti-harvesting controls                                                                                                       |         -          |  exact-match only, no listing   | check (explicit `/lookup/resolve` and `/_analytics/lookup` rate limits) |
| Rate limiting                                                                                                                         |         -          |                -                |                                  check                                  |

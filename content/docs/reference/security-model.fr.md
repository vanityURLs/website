...
| Allowlist de méthodes HTTP (`GET`/`HEAD`/`OPTIONS`, plus exceptions `POST` lookup)                                                                       |         -          |                         check                          |                                  check                                   |
...
| Contrôles anti-harvesting lookup                                                                                                                         |         -          | check (Turnstile, exact-match seulement, pas de liste) | check (rate limits explicites `/lookup/resolve` et `/_analytics/lookup`) |
| Rate limiting                                                                                                                                            |         -          |                           -                            |                                  check                                   |
...
- **Une grande partie de la posture vit à l'edge.** TLS, application HTTPS, rate limiting, contrôles bot, Access, Turnstile et premier blocage des probes scanner sont des réglages Cloudflare. Ils sont documentés dans [Protection réseau](/fr/docs/customize/network-protection/) et suivis dans `data/cloudflare-protection-defaults.json`, mais ne sont pas tous du code applicatif.

## Secrets et données sensibles

- Les secrets Worker vivent dans les secrets runtime Cloudflare, comme `CF_ACCESS_AUD` quand Access est activé et `V8S_TURNSTILE_SECRET_KEY` quand Turnstile lookup est activé. `CF_ACCESS_TEAM_DOMAIN` et `V8S_TURNSTILE_SITE_KEY` sont des variables runtime non secrètes.

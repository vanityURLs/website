...
| Chemins opérationnels protégés (`/<lang>/_stats`, `/<lang>/_tests`) | Mainteneur authentifié                           | Worker, derrière Cloudflare Access                   | Échoue fermé : `503` quand Access n'est pas configuré, `403` sans JWT valide.                                                                                                                                |
...
| Assets runtime privés cachés (`v8s.json`, blocklist, site config, manifeste des assets custom)                                                           |         -          |            check (`404`)            |              check (`_headers` no-index/no-store fallback)               |
...
| Contrôles anti-harvesting lookup                                                                                                                         |         -          | exact-match seulement, pas de liste | check (rate limits explicites `/lookup/resolve` et `/_analytics/lookup`) |
| Rate limiting                                                                                                                                            |         -          |                  -                  |                                  check                                   |

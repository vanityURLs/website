...
L'anglais (`en`) est la langue principale et de fallback lorsqu'une page localisée n'est pas actuellement [supportée](https://www.vanityurls.link/fr/docs/reference/i18n/) ou installée pendant le setup.

| Chemin | Ce que cela prouve |
|---|---|
| [v8s.link](https://v8s.link) | Page d'accueil par défaut pour chercher ou ouvrir les liens courts |
| [v8s.link/expand/](https://v8s.link/expand/) | Prévisualiser la destination d'un lien court sans l'ouvrir |
...
Les liens actuels sont stockés dans [`custom/v8s-links.txt`](https://github.com/vanityURLs/v8s.link/blob/main/custom/v8s-links.txt). Chaque ligne non commentée utilise le format documenté dans [Format des liens](/fr/docs/reference/link-format/) : `slug|target|state|title|description|tags|owner|expires_at|notes`

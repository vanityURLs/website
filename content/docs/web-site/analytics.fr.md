...
| -------- | ----------- |
| `UMAMI_WEBSITE_ID` | Secret runtime Cloudflare |
...
3. Confirmez que `UMAMI_ENDPOINT` pointe vers le bon endpoint Umami.
4. Consultez **Workers Logs** dans Cloudflare pour la requête HTML.
5. Testez depuis un navigateur qui n'est pas connecté au tableau de bord Umami.
6. Comparez les événements nommés dans l'onglet **Events** d'Umami avant de supposer que les chiffres Pages sont incorrects.
